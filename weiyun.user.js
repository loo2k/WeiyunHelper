...
(function () {
  ...
  var useRpc = GM_getValue('useRpc', false);
  let useRpcId = GM_registerMenuCommand(`[${useRpc ? "✔" : "✖"}]使用 RPC 下载`, useRpc_callback)

  function useRpc_callback() {
    useRpc = !useRpc;
    GM_setValue('useRpc', useRpc);
    GM_unregisterMenuCommand(useRpcId);
    useRpcId = GM_registerMenuCommand(`[${useRpc ? "✔" : "✖"}]使用 RPC 下载`, useRpc_callback);
  }

  var rpc = GM_getValue('rpc', 'http://localhost:6800')
  var rpcUrl = new URL(rpc).origin;
  var token = new URL(rpc).username;
  let rpcId = GM_registerMenuCommand(`RPC 地址: ${rpc}`, rpc_callback)

  function rpc_callback() {
    const result = prompt('请输入 RPC 地址 (例如: http://token@localhost:6800)', rpc);
    if (!result) return;
    rpc = result;
    rpcUrl = new URL(rpc).origin;
    token = new URL(rpc).username;
    GM_setValue('rpc', rpc);
    GM_unregisterMenuCommand(rpcId);
    rpcId = GM_registerMenuCommand(`RPC 地址: ${rpc}`, rpc_callback);
  }
  ...
  const handleResp2Aria2 = (ret) => {
    ...
    const ariaNgUrl = `http://aria2.pigfly.im/ariang/#!/new/task?url=${base64.urlencode(downloadUrl)}&header=Cookie:${cookieName}=${cookieValue}&out=${encodeURI(fileName)}`;
    ...
    if (useRpc) {
      ...
      fetch(`${rpcUrl}/jsonrpc`, rpcOptions)
        .then((res) => res.json())
        .then((res) => {
          if (res.error) {
  ...
  // 微云文件分享页面注入脚本模块
  location.host === 'share.weiyun.com' && webpackJsonp([7892], {
